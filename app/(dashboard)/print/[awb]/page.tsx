...
export default function PrintLabelPage() {
  ...
  const printAll = searchParams?.get("all") === "true"; // 🚀 Check if it's the Master Link
  ...
  // If ?all=true, create an array of [1, 2, 3...]. Otherwise, just render the specific piece requested.
  ...
  return (
    <div className="min-h-screen bg-[#0a101f] p-8 flex flex-col items-center justify-start">
      ...
      <div className="mb-6 flex gap-4 print:hidden">
        ...
        >
            <Printer size={18}/> Print {printAll ? `All ${totalPackages} Labels` : "Label"}
        </button>
        <button 
            onClick={handleClose} 
      ...
      {/* 🚀 Loop through and render the labels */}
      {labelsToRender.map((labelData, index) => {
        ...
        return (
          <div 
            key={index} 
            // 🚀 The 'break-after-page' CSS tells the printer to cut the sticker here!
            className={`bg-white text-black w-[400px] h-[600px] border-2 border-black p-4 flex flex-col box-border shadow-2xl print:shadow-none print:m-0 print:border-none print:w-full print:h-full ${printAll ? 'mb-8 print:break-after-page' : ''}`}
          >
            {/* 🟢 TOP SECTION */}
